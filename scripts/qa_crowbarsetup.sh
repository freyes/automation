...
function prepare_cloudupgrade()
{
    ...
    ### This needs some hardening and review
    # We need to make sure client nodes are in a up-to-date state
    addupdaterepos
    runupdate
    ...
    # TODO: think about cloudtup repo
...
function cloudupgrade_1st()
{
    ...
    echo 'y' | suse-cloud-upgrade upgrade &> ~/cloudupgrade_1st_log
    if [[ $? != 0 ]]; then
...
function cloudupgrade_2nd()
{
    # Upgrade Admin node
    zypper --non-interactive up -l
    echo 'y' | suse-cloud-upgrade upgrade &> ~/cloudupgrade_2nd_log
    if [[ $? != 0 ]]; then
      echo "Upgrade failed with $?"
      exit $?
    fi
